...
<template>
    <div>
        ...
        <div class="castCardContainer">
            <CastCard v-for="item in props.castObjArray.slice(0, endLength)" :castName="item.name"
                :castCharacterName="item.character" :castImage="item.profile_path" :key="item.cast_id" />
            <!-- <CastCard v-for="item in castObjArray.slice(8,)" :key="item.cast_id" /> -->
        </div>
        <div class="castCardContainerPanel">

        </div>
        <Button rounded outlined class="mt-[10px] mb-[10px]" v-if="castObjArray.length > 8" @click="toggleEndLength"
